...
We use hydraulic pull testing equipment that applies a continuous, increasing tensile load to an installed anchor until failure. The equipment records both the applied force and anchor displacement throughout the entire test, producing a complete load-displacement curve. This gives us far more information than a simple pass/fail proof load test — we can see exactly how the anchor behaves at every stage of loading, and precisely how it fails.
...
### Failure Modes by Anchor

The SP10 and SP12 typically exhibit adhesive failure — the bond gives way before the hardware. The SP18, which uses a heavier anchor body, can exhibit either failure mode depending on installation and asphalt conditions. We have test data showing both, and present examples of each below.
...
Asphalt surface temperatures above 110°F (43°C) during operation reduce anchor holding strength. At elevated temperatures, the asphalt itself softens, which weakens the bond between the cured adhesive and the surrounding pavement. This is not an installation concern — anchors can be installed at any temperature the adhesive will cure. The reduction occurs when sustained loads are applied while the pavement surface is hot. See our [hot weather procedures](/installation/bad-weather/) for guidance.
