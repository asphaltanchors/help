...
Hot pavement reduces the support that the asphalt provides around the cured adhesive and anchor. In our Sonoma PG64-16 test pavement, all anchor models tested in the 120°F temperature bucket retained approximately 58–62% of their average pull capacity in the 70°F reference bucket. The 120°F bucket is the highest tested so far.

This is an operating-temperature effect, separate from the adhesive temperature limits during installation. The result is not a universal derating factor: binder grade, mix characteristics, pavement condition, and temperature through the anchor depth can change performance. See [High Pavement Temperature and Anchor Capacity](/testing/high-pavement-temperature/) for the measured result and its limitations.
...
Anchor strength is a function of the product, the installation, and the operating conditions. A properly installed SP12 under reference test conditions will hold more than 4,000 lbs. The same anchor installed in a wet, dirty hole may not even reach its rated 2,000 lbs. Similarly, a properly installed anchor under load in hot pavement may have reduced pull capacity.
